...
## 下载离线资源
在开始使用离线方式安装Kubernetes前，可以先下载好了离线资源，也可以在创建好集群环境后再下载离线资源，请查看离线资源下载文档：[下载离线资源](download-resource.md)
...
```
至此`mycluster`集群环境已经创建好了，集群环境创建成功后，可以根据需求修改 `./inventories/mycluster/group_vars/all.yml` 文件来自定义Kubernetes集群安装的配置，如组件、网络插件版本等。

> 如果修改了组件版本，需要重新下载该组件对应版本的离线资源，见 [下载离线资源](#下载离线资源)
