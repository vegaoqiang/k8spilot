...
如果您使用k8spilot docker镜像离线方式安装Kubernetes集群，见: [离线安装](getting-started-docker-offline.md)
...
>3. `/tmp/resources`行的映射关系将容器中的k8spilot下载的文件映射到宿主机上，起到缓存文件作用，安装多环境集群时可重复使用，k8spilot将使用缓存文件从而条过在线下载，加快部署Kubernetes速度。
